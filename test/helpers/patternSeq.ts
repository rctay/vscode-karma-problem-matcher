...
chai.use((_chai: any, utils: any) => {
  ...
  Assertion.addProperty('haveAnEntry', function(this: any) {
    ...
    new Assertion(subject).to.be.an('array')
      .that.is.not.empty;
  });

  function findMatch(lines: string[], pattern: RegExp, from = 0) {
    let i, match;
    for (i = from; i < lines.length && (match = lines[i].match(pattern)) === null; i++) {
    }
    return {i, match};
  }
  ...
  Assertion.addProperty('nextEntryAndPatternMatch', function (this: any) {
    const subject: string[] = this._obj;
    const {entryIndex, patternIndex, patterns}: StrictSequenceState = utils.flag(this, '__strictSequenceState');
  ...
  Assertion.addProperty('patternsExhausted', function(this: any) {
    const {patternIndex, patterns}: StrictSequenceState = utils.flag(this, '__strictSequenceState');
    this.assert(
  ...
  Assertion.addMethod('matchRegexp', function(this: any, problemPattern: ProblemPatternP) {
    ...
    const { entryIndex = undefined }: RelaxedSequenceState = utils.flag(this, '__relaxedSequenceState') || {};
    ...
    const updatedState: RelaxedSequenceState = {
      entryIndex: i
    };
